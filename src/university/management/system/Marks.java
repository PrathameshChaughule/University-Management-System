package university.management.system;

import java.awt.*;
import javax.swing.*;
import java.sql.*;
import java.awt.event.*;

public class Marks extends JFrame implements ActionListener{
    
    String rollno;
    JButton cancel;
    
    Marks(String rollno){
        this.rollno = rollno;
        
        setSize(500, 600);
        setLocation(500, 100);
        
        setLayout(null);
        
        getContentPane().setBackground(Color.WHITE);
        
        JLabel heading = new JLabel("University Of Mumbai");
        heading.setBounds(130, 10, 500, 25);
        heading.setFont(new Font("Tahoma", Font.BOLD, 20));
        add(heading);
        
        JLabel subheading = new JLabel("Result Of Examination 2025");
        subheading.setBounds(100, 50, 500, 25);
        subheading.setFont(new Font("Tahoma", Font.BOLD, 20));
        add(subheading);
        
        JLabel lblrollno = new JLabel("Roll Number: " + rollno);
        lblrollno.setBounds(60, 100, 500, 20);
        lblrollno.setFont(new Font("Tahoma", Font.PLAIN, 18));
        add(lblrollno);
        
        JLabel lblsemester = new JLabel();
        lblsemester.setBounds(60, 130, 500, 20);
        lblsemester.setFont(new Font("Tahoma", Font.PLAIN, 18));
        add(lblsemester);
        
        JLabel subject = new JLabel("Subject");
        subject.setBounds(120, 190, 100, 21);
        subject.setFont(new Font("Tahoma", Font.BOLD, 19));
        add(subject);
        
        JLabel sub1 = new JLabel();
        sub1.setBounds(130, 220, 100, 20);
        sub1.setFont(new Font("Tahoma", Font.PLAIN, 18));
        add(sub1);

        JLabel sub2 = new JLabel();
        sub2.setBounds(130, 250, 100, 20);
        sub2.setFont(new Font("Tahoma", Font.PLAIN, 18));
        add(sub2);

        JLabel sub3 = new JLabel();
        sub3.setBounds(130, 280, 100, 20);
        sub3.setFont(new Font("Tahoma", Font.PLAIN, 18));
        add(sub3);

        JLabel sub4 = new JLabel();
        sub4.setBounds(130, 310, 100, 20);
        sub4.setFont(new Font("Tahoma", Font.PLAIN, 18));
        add(sub4);

        JLabel sub5 = new JLabel();
        sub5.setBounds(130, 340, 100, 20);
        sub5.setFont(new Font("Tahoma", Font.PLAIN, 18));
        add(sub5);
        
        JLabel sub6 = new JLabel();
        sub6.setBounds(130, 370, 100, 20);
        sub6.setFont(new Font("Tahoma", Font.PLAIN, 18));
        add(sub6);
        
        JLabel marks = new JLabel("Marks");
        marks.setBounds(290, 190, 100, 21);
        marks.setFont(new Font("Tahoma", Font.BOLD, 19));
        add(marks);
        
        JLabel marks1 = new JLabel();
        marks1.setBounds(300, 220, 100, 20);
        marks1.setFont(new Font("Tahoma", Font.PLAIN, 18));
        add(marks1);

        JLabel marks2 = new JLabel();
        marks2.setBounds(300, 250, 100, 20);
        marks2.setFont(new Font("Tahoma", Font.PLAIN, 18));
        add(marks2);

        JLabel marks3 = new JLabel();
        marks3.setBounds(300, 280, 100, 20);
        marks3.setFont(new Font("Tahoma", Font.PLAIN, 18));
        add(marks3);

        JLabel marks4 = new JLabel();
        marks4.setBounds(300, 310, 100, 20);
        marks4.setFont(new Font("Tahoma", Font.PLAIN, 18));
        add(marks4);

        JLabel marks5 = new JLabel();
        marks5.setBounds(300, 340, 100, 20);
        marks5.setFont(new Font("Tahoma", Font.PLAIN, 18));
        add(marks5);

        JLabel marks6 = new JLabel();
        marks6.setBounds(300, 370, 100, 20);
        marks6.setFont(new Font("Tahoma", Font.PLAIN, 18));
        add(marks6);

        

        try {
            Conn c = new Conn();
            
            ResultSet rs1 = c.s.executeQuery("select * from subject where rollno='"+rollno+"'");
            while(rs1.next()){
                sub1.setText(rs1.getString("subject1"));
                sub2.setText(rs1.getString("subject2"));
                sub3.setText(rs1.getString("subject3"));
                sub4.setText(rs1.getString("subject4"));
                sub5.setText(rs1.getString("subject5"));
                sub6.setText(rs1.getString("subject6"));
            }
            
            ResultSet rs2 = c.s.executeQuery("select * from marks where rollno='"+rollno+"'");
            while(rs2.next()){
                marks1.setText(rs2.getString("marks1"));
                marks2.setText(rs2.getString("marks2"));
                marks3.setText(rs2.getString("marks3"));
                marks4.setText(rs2.getString("marks4"));
                marks5.setText(rs2.getString("marks5"));
                marks6.setText(rs2.getString("marks6"));
                lblsemester.setText("Semester: " + rs2.getString("semester"));
            }
        } catch (Exception e){
            e.printStackTrace();
        }
        
        cancel = new JButton("Back");
        cancel.setBounds(250, 500, 120, 25);
        cancel.setBackground(Color.BLACK);
        cancel.setForeground(Color.WHITE);
        cancel.addActionListener(this);
        cancel.setFont(new Font("Tahoma", Font.BOLD, 15));
        add(cancel);
        
        setVisible(true);
    }
    
    public void actionPerformed(ActionEvent ae){
        setVisible(false);
    }
    
    public static void main(String[] args){
        new Marks("");
    }
}
